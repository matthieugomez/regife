...
program define innerregife, eclass 
	...
	fast ///
	Absorb(string) TOLerance(real 1e-7) MAXIterations(int 10000) VERBose partial * ///
	]
	...
	* until reghdfe 3.0 is on ssc
		...
		* iterate 
		mata: info = iteration("`py'","`res'", "`px'", "`wvar'", "`g1'", "`g2'", `N', `T', `dimension', `tolerance', `maxiterations', "`b'", `touse_first', `touse_last', "`id1gen'", "`tfactorlist'", "`verbose'")
		local iter = r(N)
		...
		if "`fast'" == ""{
			...
			if "`partial'" == ""{
				qui reghdfe `y' `x' `wt'  in `touse_first'/`touse_last', a( `absorb' `id1'#c.(`tfactorlist')) `options'
				tempname df_a
